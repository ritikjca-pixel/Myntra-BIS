...
PARALLEL_PRODUCT_WORKERS = int(os.getenv("PARALLEL_PRODUCT_WORKERS", "5"))
...
def warm_up_session(session, progress_callback=None):
    """
    Visits the homepage/clothing page once per SESSION to establish cookies,
    before hitting the search API or a product page. State is tracked on the
    session object itself (session._warmed_up) rather than a module-level
    global, because with parallel product checks each thread has its own
    session - a single global flag would only warm up the first one and
    leave the rest without cookies.
    """
...
def _check_product_worker(url, title, progress_callback):
    """
    Runs in its own thread with its own curl_cffi session (sessions aren't
    safe to share across threads). Used to check several single-product
    URLs concurrently instead of one-by-one.
    """
    try:
        thread_session = requests.Session(impersonate="chrome110")
        return check_single_product(thread_session, url, title, progress_callback)
    except Exception as e:
        print(f"  [PDP worker error] {url}: {e}", file=sys.stderr)
        if progress_callback:
            progress_callback(f"Error checking {url[:50]}: {e}")
        return None
...
def scrape_all(urls, progress_callback=None):
    ...
    # Split into single-product (PDP) links and category/listing links up front.
    pdp_entries = []      # (title, url)
    category_entries = []  # (title, url)
    ...
    for entry in urls:
        ...
        if is_product_url(url):
            pdp_entries.append((title, url))
        else:
            category_entries.append((title, url))

    total_pdp = len(pdp_entries)

    # --------------------------------------------------------------
    # SINGLE-PRODUCT URLs: checked CONCURRENTLY.
    # This is the part that used to take ~5-6s per product sequentially
    # (25-30s for 5 products). Running them in parallel brings a full
    # pass down to roughly the time of the SLOWEST single product check,
    # so a fast-selling restock window is far less likely to be missed.
    # --------------------------------------------------------------
    if pdp_entries:
        if progress_callback:
            progress_callback(f"Checking {total_pdp} product(s) in parallel...")

        max_workers = min(PARALLEL_PRODUCT_WORKERS, total_pdp) or 1
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            future_to_entry = {
                executor.submit(_check_product_worker, url, title, progress_callback): (title, url)
                for title, url in pdp_entries
            }
            completed = 0
            for future in as_completed(future_to_entry):
                ...
                completed += 1
                if result:
                    single_product_rows.append(result)
                if progress_callback:
                    progress_callback(f"Checked {completed}/{total_pdp} products...")

    # --------------------------------------------------------------
    # CATEGORY/LISTING URLs: kept SEQUENTIAL.
    # These involve pagination, auto-splitting, and session warm-up/cookie
    # state that's easier to reason about (and less likely to trip Myntra's
    # rate limiting) one at a time.
    # --------------------------------------------------------------
    if category_entries:
        session = requests.Session(impersonate="chrome110")
        total_cat = len(category_entries)
        for idx, (title, url) in enumerate(category_entries, 1):
            if progress_callback:
                progress_callback(f"Starting category link {idx} of {total_cat}...")

            rows, blocked, _ = scrape_url(session, url, title, progress_callback)
